...
#include <string.h>
	...
	/* TODO: USE NANO GL */
	#include <qgl.h>
	#include <qgl_hardlinked.h>
	#define qglLoadMatrixd qglLoadMatrixf
	#define double float
#else
#  include <GL/glut.h>
...
{
#ifdef ANDROID
	printf("argInit2 Skip GLUT Init\n");
#else
...
#  endif	
    ...
    printf("glDrawPixels( gImXsize, gImYsize, GL_RGB, GL_UNSIGNED_BYTE, image );\n"); // Vldimir
...
#endif
{
    ...
    glPixelStorei( GL_UNPACK_ROW_LENGTH, gImXsize*size_adjust_factor );
#else
	printf("NO glPixelStorei( GL_UNPACK_ROW_LENGTH, gImXsize*size_adjust_factor );\n");
#endif
    if( size_adjust_factor == old_size_adjust_factor ) {
#if (AR_DEFAULT_PIXEL_FORMAT == AR_PIXEL_FORMAT_ARGB)
#  ifdef AR_BIG_ENDIAN
...
#elif (AR_DEFAULT_PIXEL_FORMAT == AR_PIXEL_FORMAT_RGB)
    printf("glDrawPixels( gImXsize/2, gImYsize/2, GL_RGB, GL_UNSIGNED_BYTE, image );\n"); // Vladimir
#elif (AR_DEFAULT_PIXEL_FORMAT == AR_PIXEL_FORMAT_MONO)
...
#endif
{
    ...
    // Vladimir glPixelStorei( GL_UNPACK_ROW_LENGTH, gImXsize/2 );
...
#  error Unknown default pixel format defined in config.h
    ...
    glBindTexture( GL_TEXTURE_2D, 0 );
    glDisable( GL_TEXTURE_2D );
    // Vladimir glPixelStorei( GL_UNPACK_ROW_LENGTH, 0 );
}
